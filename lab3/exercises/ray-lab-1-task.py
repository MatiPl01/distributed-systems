...
def run_local_bubble_sort(arr):
    results = [bubble_sort(arr) for _ in range(os.cpu_count())]
    return results

def run_remote_bubble_sort(arr):
...
cProfile.run("run_local_bubble_sort(array)")

print("remote bubble sort run")
cProfile.run("run_remote_bubble_sort(array)")

shuffle(array)
sorted_arr_ref = bubble_sort_distributed.remote(array)
result = ray.get(sorted_arr_ref)
